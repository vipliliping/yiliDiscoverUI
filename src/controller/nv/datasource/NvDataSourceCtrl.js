'use strict';
discovery.controller('NvDataSourceCtrl', function ($scope, $http, ModalUtils, $uibModal, $filter, $q) {
    "ngInject";
    ...
    $scope.deleteDs = function (ds) {
        ...
        var promiseDs = $http.get("dashboard/getAllDatasetList.do").then(function (response) {
            ...
            for (var i = 0; i < response.data.length; i++) {
                if (response.data[i].data && response.data[i].data.datasource == ds.id) {
    ...
    $scope.test = function () {
        ...
        $uibModal.open({
            templateUrl: 'src/view/config/modal/test.html',
