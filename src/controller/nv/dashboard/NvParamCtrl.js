'user strict';
discovery.controller('NvParamCtrl', function ($scope, $uibModal, $http) {
    "ngInject";
    ...
    $scope.init = function () {
        ...
        $scope.param.selects = [];
        $scope.param.type = '=';
        if(typeof $scope.param.values === 'undefined')
            $scope.param.values = [];
        if ($scope.param.paramType == 'slider') {
            ...
            _.each($scope.param.col, function (c) {
                ...
                if (_.isUndefined(c.datasetId)) {
                    ...
                    //         if (widget.widget.data && widget.widget.data.datasetId == c.widgetId) {
                    //             p = {
    ...
    $scope.deleteParam = function (row,$index) {
