 ...
 */
discovery.controller('paramSelector', function ($scope, $uibModalInstance, dataService, param, filter, getSelects, ok) {
    "ngInject";
    ...
    $scope.addInputParam = function () {
        var str = $.trim($scope.paramInput);
        if (str != "") {
            var found = false;
            for (var i = 0; i < $scope.param.values.length; i++) {
                if ($scope.param.values[i] == str) {
                    found = true
                    break;
                }
            }
            if (!found)
                $scope.param.values.push($scope.paramInput);
        }
    };
