 ...
 */
discovery.controller('userAdminCtrl', function ($scope, $http, ModalUtils, $filter) {
    "ngInject";
    ...
    //字面意思获取角色用户列表。页面并没有用到该变量--userRoleList
    ...
    //组装各个tab标签内的树
    $scope.tree = {menu: {}, board: {}, datasource: {}, dataset: {}, widget: {}, job: {}, brain: {}};
    ...
    //获取tab-brain的树数据
    var getBrainList = function () {
        return $http.get("mindmap/getMindmapList.do").success(function (response) {
            _.each(buildNodeByCategory(response, 'Brain', 'brain', 'fa fa-clock-o'), function (e) {
                $scope.tree.brain.resList.push(e);
            });
        });
    };
    ...
    $scope.changeResSelect = function () {
        ...
        changeResSelectByTree($scope.tree.brain);
    };

    //初始化右侧树形数据，根据选中的角色或用户更改每个节点的状态
