discovery.controller('renderCtrl', function ($timeout, $rootScope, $scope, $state, $location, $http, ModalUtils, chartService) {
    "ngInject";
    $scope.loading = true;
    $scope.l = 1;
    ...
    $scope.load = function (reload) {
        ...
        $http.get("dashboard/getBoardData.do?id=" + $location.search().id + '&token=' + getQueryString('yili-token')).success(function (response) {
            $scope.loading = false;
